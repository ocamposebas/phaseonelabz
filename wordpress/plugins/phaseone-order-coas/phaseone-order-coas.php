 ...
 * Description: Records exact WooCommerce fulfillment lots and exposes their COAs to the purchasing customer.
 * Version: 1.0.3
...
define( 'PHASEONE_ORDER_COAS_VERSION', '1.0.3' );
...
function phaseone_order_coas_candidates_for_item( $item, $catalog ) {
	...
	usort(
		...
		static function ( $left, $right ) {
			...
			return strcasecmp( $right['testing_date'], $left['testing_date'] );
		}
	);
	return $candidates;
}
				...
				if ( ! empty( $candidates ) ) {
					$items[] = array_merge(
						...
						array(
							...
							'message'     => 'COA pending — lot information has not been assigned to this order.',
							'coa'         => null,
						)
					);
				}
	...
	<div class="wrap poco-shell">
		<header class="poco-header">
			<div>
				...
				<p>Confirm the physical lot packed for each order item. The current shipping lot is a suggestion only.</p>
			</div>
