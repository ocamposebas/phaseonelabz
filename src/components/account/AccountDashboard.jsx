import "./AccountDashboard.styles.css";
...
const AccountAddressManager = lazy(() => import("./AccountAddressManager.jsx"));
const AccountCoas = lazy(() => import("./AccountCoas.jsx"));
const AffiliateApplicationPanel = lazy(() =>
  import("./AccountAffiliatePanel.jsx")
);

const POINTS_PER_REWARD = 500;
const CREDIT_PER_REWARD = 5;
const ORDER_RENDER_BATCH_SIZE = 6;

const dashboardTabs = [
  { id: "overview", label: "Overview", shortLabel: "Home", icon: LayoutDashboard },
  { id: "personal", label: "Profile", shortLabel: "Profile", icon: User },
  { id: "rewards", label: "Rewards", shortLabel: "Rewards", icon: Gift },
  { id: "coas", label: "COAs", shortLabel: "COAs", icon: FileCheck2 },
...
function getLocationAccountTab(fallback = "overview") {
  if (typeof window === "undefined") return fallback;

  if (/^\/account\/coas\/?$/i.test(window.location.pathname)) return "coas";

  const hashTab = window.location.hash.replace(/^#/, "").toLowerCase();
  return dashboardTabs.some((tab) => tab.id === hashTab) ? hashTab : fallback;
}

function getAccountTabUrl(tabId) {
  if (tabId === "coas") return "/account/coas";
  if (tabId === "overview") return "/account";
  return `/account#${tabId}`;
}
...
export default function AccountDashboard({ initialTab = "overview" }) {
  ...
  const safeInitialTab = dashboardTabs.some((tab) => tab.id === initialTab)
    ? initialTab
    : "overview";
  ...
  const [visibleOrderCount, setVisibleOrderCount] = useState(
    ORDER_RENDER_BATCH_SIZE
  );
  ...
  const renderedOrders = useMemo(
    () => visibleOrders.slice(0, visibleOrderCount),
    [visibleOrders, visibleOrderCount]
  );
  ...
  useEffect(() => {
    const syncTabFromLocation = () => {
      setActiveTab(getLocationAccountTab(safeInitialTab));
    };
  ...
  const selectAccountTab = (tabId) => {
    const nextTab = dashboardTabs.some((tab) => tab.id === tabId)
      ? tabId
      : "overview";
    setActiveTab(nextTab);

    if (typeof window !== "undefined") {
      const nextUrl = getAccountTabUrl(nextTab);
      const currentUrl = `${window.location.pathname}${window.location.hash}`;
      if (currentUrl !== nextUrl) {
        window.history.pushState({ accountTab: nextTab }, "", nextUrl);
      }
    }
  };
  ...
  const handleLogout = async () => {
    ...
    setRedeemMessage("");
    setVisibleOrderCount(ORDER_RENDER_BATCH_SIZE);
    setTrackingByOrder({});
  ...
  return (
    <section className="account-page-shell relative isolate min-h-[100svh] overflow-x-hidden bg-[#020617] px-4 pb-32 pt-10 text-white sm:px-6 sm:py-16 lg:px-8 lg:pb-10 lg:pt-8">
      ...
      <div className="relative mx-auto max-w-[1120px]">
        ...
        <DashboardMenu activeTab={activeTab} setActiveTab={selectAccountTab} />
              ...
              <SectionCard className="hidden sm:block">
                ...
                <div className="grid">
                  <AccountShortcut
                    ...
                    onClick={() => selectAccountTab("personal")}
                  />
                  ...
                  <AccountShortcut
                    ...
                    onClick={() => selectAccountTab("rewards")}
                  />
                  ...
                  <AccountShortcut
                    ...
                    onClick={() => selectAccountTab("affiliate")}
                  />
                </div>
              </SectionCard>
            </div>
        ...
        {activeTab === "coas" && (
          <Suspense
            fallback={
              <div className="flex min-h-40 items-center justify-center gap-3 py-10 text-sm text-cyan-100">
                <Loader2 size={18} className="animate-spin" />
        ...
        {activeTab === "orders" && (
          <div className="py-2 sm:py-3">
            ...
            {visibleOrders.length === 0 ? (
              ...
              <div className="space-y-4 sm:space-y-5">
                {renderedOrders.map((order, orderIndex) => {
                    ...
                    >
                      ...
                      <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
                        ...
                        <div className="grid grid-cols-2 gap-2 lg:flex">
                          {hasPoints && (
                            <a
                              href={`/account/coas?order=${encodeURIComponent(
                                order.number || order.id
                              )}`}
                              className="inline-flex min-h-[46px] items-center justify-center gap-2 rounded-2xl border border-white/[0.09] bg-white/[0.025] px-4 text-[9px] font-black uppercase tracking-[0.15em] text-slate-300 transition hover:border-cyan-200/20 hover:text-cyan-100 lg:min-h-[40px] lg:rounded-xl"
                            >
                              <FileCheck2 size={14} />
                              COAs
                            </a>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setVisibleOrderCount((current) =>
                        Math.min(
                          current + ORDER_RENDER_BATCH_SIZE,
                          visibleOrders.length
                        )
                      )
                    }
                    className="inline-flex min-h-[48px] w-full items-center justify-center gap-2 rounded-2xl border border-cyan-200/15 bg-cyan-300/[0.055] px-5 text-[9px] font-black uppercase tracking-[0.16em] text-cyan-100 transition hover:border-cyan-200/25 hover:bg-cyan-300/[0.09] hover:text-white"
                  >
                    Show more orders
                    <Plus size={14} />
                  </button>
