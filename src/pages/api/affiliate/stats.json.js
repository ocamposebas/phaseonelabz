...
export async function POST({ request }) {
  try {
      ...
      return jsonResponse(
        {
          ok: false,
          message:
            "Affiliate stats bridge is not configured. Check WORDPRESS_URL and PHASEONE_AFFILIATE_BRIDGE_SECRET in .env.",
          env: {
    ...
    const endpoint = `${String(wordpressUrl).replace(
      /\/$/,
      ""
    )}/wp-json/phaseone/v1/coupon-affiliate/stats`;
    ...
    return jsonResponse(
      {
        ok: true,
        message: data?.message || "Affiliate stats loaded.",
        data,
      },
      200
    );
    ...
    return jsonResponse(
      {
        ok: false,
        message: error?.message || "Affiliate stats could not be loaded.",
        debug: {
