  ...
  try {
    ...
    if (!rawBody) {
      return jsonResponse(
        {
          ok: false,
          message: "The request body is empty. No account data was sent.",
          debug: {
      ...
      return jsonResponse(
        {
          ok: false,
          message:
            "Affiliate status bridge is not configured. Check WORDPRESS_URL and PHASEONE_AFFILIATE_BRIDGE_SECRET in .env.",
          env: {
    ...
    const endpoint = `${String(wordpressUrl).replace(
      /\/$/,
      ""
    )}/wp-json/phaseone/v1/coupon-affiliate/status`;
    ...
    const wpResponse = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        accountId: body?.accountId || body?.user_id || body?.id || "",
    ...
    return jsonResponse(
      {
        ok: true,
        message: data?.message || "Affiliate status loaded.",
        data,
      },
      200
    );
    ...
    return jsonResponse(
      {
        ok: false,
        message: error?.message || "Affiliate status could not be loaded.",
        debug: {
